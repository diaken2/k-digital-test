...
import { getImagePlaceholderFromMime, phoneNumber, savePdf } from '../../utils'
...
const OrderDetail = (): JSX.Element => {
  ...
  const [firstDocDownloadLoading, setFirstDocDownloadLoading] = useState(false)
  const [firstDocPrintLoading, setFirstDocPrintLoading] = useState(false)
  const [secondDocDownloadLoading, setSecondDocDownloadLoading] =
    useState(false)
  const [secondDocPrintLoading, setSecondDocPrintLoading] = useState(false)
  const [thirdDocDownloadLoading, setThirdDocDownloadLoading] = useState(false)
  const [thirdDocPrintLoading, setThirdDocPrintLoading] = useState(false)
  ...
  const [visibleSignSec, setVisibleSignSec] = useState<boolean>(false)
  const [visibleSignSecAct, setVisibleSignSecAct] = useState<boolean>(false)
  const [visibleSignSecCom, setVisibleSignSecCom] = useState<boolean>(false)
  ...
  const printFirstDoc = useRef<any>(null)
  const printSecondDoc = useRef<any>(null)
  const printThirdDoc = useRef<any>(null)
  ...
  //Функция для сохранения блока в виде пдф файла
  const saveBlockAsPdf = (block: string): void => {
    if (!block) {
      return
    }
    const distBlockObjSetFalse: any = {
      blockOneWithDoc: () => setVisibleSignSec(true),
      blockTwoWithDoc: () => setVisibleSignSecAct(true),
      blockThreeWithDoc: () => setVisibleSignSecCom(true),
    }
    const distBlockObjSetTrue: any = {
      blockOneWithDoc: () => setVisibleSignSec(false),
      blockTwoWithDoc: () => setVisibleSignSecAct(false),
      blockThreeWithDoc: () => setVisibleSignSecCom(false),
    }
    distBlockObjSetFalse[block]()
    new Promise((res, rej) => {
      //Эта функция обращается к другой функции - savePdf, которую я импортирую из utils.ts
      try {
        res(savePdf(block, data?.id))
      } catch (error: any) {
        rej(error)
      }
    })
      .then(() => {
        distBlockObjSetTrue[block]()
      })
      .catch((res: any) => {
        dispatch({
          type: 'toast',
          toast: {
            message:
              'Произошла ошибка при скачивании документа, попробуйте позже',
            status: res.status,
          },
        })
      })
              ...
              >
                <CForm>
                  ...
                  <div
                    className={
                      visibleSignSec === true
                        ? 'show-sign-section-when-download'
                        : 'sign-section'
                    }
                  >
                    <div
                  ...
                  <div
                    ...
                    className="download-print-buttons"
                  >
                    <ReactToPrint
                      trigger={() => <CButton color="primary">Печать</CButton>}
                      content={() => printFirstDoc.current}
                    />
                    <CLoadingButton
                      loading={firstDocDownloadLoading}
                      disabled={firstDocDownloadLoading}
                      onClick={() => {
                        setFirstDocDownloadLoading(true)
                        new Promise((res) => {
                          res(saveBlockAsPdf('blockOneWithDoc'))
                        }).then(() => {
                          setTimeout(() => {
                            setFirstDocDownloadLoading(false)
                          }, 2000)
                        })
                      }}
                      color="primary"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={
                        visibleSignSecAct === true
                          ? 'show-sign-section-act-when-download'
                          : 'sign-section-act'
                      }
                    >
                      <div
                    ...
                    <div
                      ...
                      className="download-print-buttons"
                    >
                      {/*Для печати блоков использовал библиотеку react-to-print */}
                      <ReactToPrint
                        trigger={() => (
                          <CButton color="primary">Печать</CButton>
                        )}
                        content={() => printSecondDoc.current}
                      />
                      <CLoadingButton
                        loading={secondDocDownloadLoading}
                        disabled={secondDocDownloadLoading}
                        onClick={() => {
                          setSecondDocDownloadLoading(true)
                          new Promise((res) => {
                            res(saveBlockAsPdf('blockTwoWithDoc'))
                          }).then(() => {
                            setTimeout(() => {
                              setSecondDocDownloadLoading(false)
                            }, 2000)
                          })
                        }}
                        color="primary"
                ...
                <CForm>
                  ...
                  <div data-html2canvas-ignore>
                    ...
                    >
                      ...
                      ></div>
                      {/* //FIELD BAR */}
                      <div className="sign-section-com-txtarea">
                        <CFormTextarea
                          id="info"
                  ...
                  <div
                    className={
                      visibleSignSecCom === true
                        ? 'show-sign-section-com-when-download'
                        : 'sign-section-com'
                    }
                  >
                    <div
                  ...
                  >
                    ...
                    <ReactToPrint
                      trigger={() => <CButton color="primary">Печать</CButton>}
                      content={() => printThirdDoc.current}
                    />
                    <CLoadingButton
                      loading={thirdDocDownloadLoading}
                      disabled={thirdDocDownloadLoading}
                      onClick={() => {
                        setThirdDocDownloadLoading(true)
                        new Promise((res) => {
                          res(saveBlockAsPdf('blockThreeWithDoc'))
                        }).then(() => {
                          setTimeout(() => {
                            setThirdDocDownloadLoading(false)
                          }, 2000)
                        })
                      }}
                      color="primary"
